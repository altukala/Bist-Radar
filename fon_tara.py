...
def main() -> None:
    ...
    for item in items:
        ...
        rows += f"""<tr><td><span class="tag {css}">{status}</span></td><td><strong>{html.escape(str(fund.get('fund_code', '')))}</strong></td><td>{html.escape(str(fund.get('name', '')))}</td><td>{html.escape(str(fund.get('fund_type', '')))}</td><td>{text_percent(number(fund, 'return_1m'))}</td><td>{text_percent(number(fund, 'return_3m'))}</td><td>{text_percent(number(fund, 'return_ytd'))}</td><td>{text_percent(number(fund, 'return_1y'))}</td><td>{item['reason']}</td></tr>"""
